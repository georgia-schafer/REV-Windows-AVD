...
# Patch: remove PDF from Edge default associations
 Write-Host 'Patch: Remove Edge from default associations'
 $updatePath= "C:\optimize\Virtual-Desktop-Optimization-Tool-master\2009\ConfigurationFiles\DefaultAssociationsConfiguration.xml"
 $fileContents = Get-Content -path $updatePath | Where-Object {$_ -notmatch '  <Association ApplicationName="Microsoft Edge" ProgId="MSEdgePDF" Identifier=".pdf"/>'}
 $fileContents | Set-Content -Path $updatePath
 
 # Patch: disable unneeded AppxPackages
 Write-Host 'Patch: Disable unneeded AppxPackages:'
 $updatePath= "C:\optimize\Virtual-Desktop-Optimization-Tool-master\2009\ConfigurationFiles\AppxPackages.json"
 $fileContents = Get-Content -path $updatePath -raw | ConvertFrom-JSON
 $disableApps = @("Microsoft.BingNews","Microsoft.BingWeather","Microsoft.GamingApp","Microsoft.Getstarted","Microsoft.MicrosoftSolitaireCollection","Microsoft.SkypeApp","Microsoft.WindowsFeedbackHub","Microsoft.WindowsMaps","Microsoft.Xbox.TCUI","Microsoft.XboxGameOverlay","Microsoft.XboxGamingOverlay","Microsoft.XboxIdentityProvider","Microsoft.XboxSpeechToTextOverlay","Microsoft.YourPhone","Microsoft.ZuneMusic","Microsoft.ZuneVideo","Microsoft.XboxApp","Microsoft.MixedReality.Portal","Microsoft.Wallet")
 foreach($app in $fileContents) {
  if ($app.AppxPackage -contains $disableApps) {
  write-host "App matched " $app.AppxPackage
    $App.VDIState = "Disabled"
  }
 }
 $fileContents | ConvertTo-JSON | Set-Content -Path $updatePath
